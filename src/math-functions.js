...
export function sum(a, b) {
    let array = [];
    let sum = a + b;
    let stringConcat = `The sum of ${a} and ${b} is ${sum}.`;
    array.push(sum);
    array.push(stringConcat);
    return array;
}
...
export function multiply(a, b) {
    let array = [];
    let product = a * b;
    let stringConcat = `The product of ${a} and ${b} is ${product}.`;
    array.push(product);
    array.push(stringConcat);
    return array;
}
...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    let myArray = [];
    let hello = sum(a, b)[0];
    let allAdded = sum(hello, c)[0];
    let hello1 = multiply(a, b)[0];
    let allMultiplied = multiply(hello1, c)[0];
    let string1 = `${a} and ${b} and ${c} sum to ${allAdded}.`;
    let string2 = `The product of ${a} and ${b} and ${c} is ${allMultiplied}.`;
    myArray.push(allAdded);
    myArray.push(allMultiplied);
...
export function sumArrayWithThreeNumbers(sumArr) {
    let sumTotal1 = sum(sumArr[0], sumArr[1])[0];//returns 0 & 1 added 
    let sumTotal2 = sum(sumTotal1, sumArr[2])[0];//returns sum 0 & 1 added then adds 2
    
    return [sumTotal2, `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${sumTotal2} is their sum.`];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    let mulTotal1 = multiply(multArr[0], multArr[1])[0];
    let mulTotal2 = multiply(mulTotal1, multArr[2])[0];
    return [mulTotal2, `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${mulTotal2}.`];
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    let accumulator = 1;
    // console.log(accumulator, "hello FIRST");

    for(let i = 0; i < dynamicArray.length; i++) {
        // const dynamicTotal = dynamicArray[i];
        // console.log(dynamicArray[i], "POOP");
